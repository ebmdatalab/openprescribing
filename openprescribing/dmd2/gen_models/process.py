...
with open("schema_raw.csv") as f:
    lines = list(
        csv.DictReader(f, fieldnames=["table", "field", "optional", "orig_descr"])
    )
...
for line in lines:
    ...
    if line["table"] != table:
        ...
        if line["field"] == "cd" or fks[line["field"]] == table:
            ...
            if line["table"] in ["vpi", "ont", "droute", "ap_ing", "lic_route"]:
        ...
        line["decimal_places"] = re.search(
            "(\d+) decimal places", line["orig_descr"]
        ).groups()[0]
    ...
    if line["field"] == "cd":
        ...
        if (
            "date code is applic" not in descr.lower()
            and "previous code" not in descr.lower()
        ):
