...
class CommandsTestCase(TestCase):
    def test_import_dmd(self):
        ...
        self.assertEqual(amp.prescribability.desc, "Valid as a prescribable product")
        self.assertEqual(amp.vmp_non_availability.desc, "Actual Products Available")
        self.assertEqual(amp.controlled_drug_category.desc, "No Controlled Drug Status")
        self.assertEqual(vmp.tariff_category.desc, "Part VIIIA Category C")
        ...
        # From v_vpm2_XXX.xml
        self.assertQuery(
            "SELECT isid FROM dmd_vpi WHERE vpid = 22480211000001104", 426714006
        )

        # From f_vmpp2_XXX.xml
        self.assertQuery(
            "SELECT pay_catcd FROM dmd_dtinfo WHERE vppid = 22479411000001100", 3
        )

        # From f_amp2_XXX.xml
        self.assertQuery(
            "SELECT isid FROM dmd_ap_ing WHERE apid = 22479611000001102", 255859001
        )

        # From f_ampp2_XXX.xml
        self.assertQuery(
            "SELECT price FROM dmd_price_info WHERE appid = 22479711000001106", 659
        )
        ...
        self.assertQuery(
            "SELECT gtin FROM dmd_gtin WHERE appid = 22479711000001106", "5051562030603"
        )
        ...
        self.assertQuery(
            "SELECT count(*) FROM dmd_vmp WHERE vpid = 22480211000001104", 0
        )
        ...
        self.assertQuery(
            "SELECT vpid FROM dmd_vmpp WHERE vppid = 22479411000001100", 12345
        )
