...
EXTRA_INDEXES = ["parallel_import", "lic_authcd", "pres_statcd", "discdt", "nurse_f"]
    ...
    with connection.cursor() as cursor:
        ...
        for name, coltype in info["columns"]:
            ...
            if name == PRIMARY_KEYS.get(info["table_name"], ""):
                row_sql += " PRIMARY KEY"
            elif any([name in x for x in PRIMARY_KEYS.values() + EXTRA_INDEXES]):
                indexes.append(name)
            cols.append(row_sql)
        sql += ", ".join(cols)
        sql += ");"
...
def get_table_info(source_directory, schema_names):
    ...
    for schema_name in schema_names:
        ...
        tables = root.findall("xs:element/xs:complexType/xs:sequence/xs:element", ns)
        ...
        for table in tables:
            ...
            if len(root.findall(".//xs:all", ns)) > 0:
                ...
                columns = root.findall(
                    ".//xs:complexType[@name='%s']/xs:all/xs:element" % schema_name, ns
                )
            else:
    ...
    for f in to_process:
        ...
        with open(test_file, "w") as outfile:
            if "gtin" in f:
                ...
                outfile.write(etree.tostring(root, encoding="utf8", method="xml"))
            else:
                doc = etree.parse(f)
                root = doc.getroot()
                ns = (
...
def update_product_references():
    ...
    with connection.cursor() as cursor:
        cursor.execute("SELECT vpid, vpidprev FROM dmd_vmp WHERE vpidprev IS NOT NULL")

        values = cursor.fetchall()

        cursor.executemany(
            "UPDATE dmd_tariffprice SET product_id = %s WHERE product_id = %s", values
        )
...
class Command(BaseCommand):
    args = ""
    help = (
        "Imports dm+d dataset to local SQL tables. Download it from "
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--extract_test",
            help=(
                "Write test fixtures to source_directory, based on the "
