...
class TestFetchAndImportNCSOConcesions(TestCase):
    ...
    def test_fetch_and_import_ncso_concessions(self):
        ...
        with patch(patch_path + ".Command.download_archive") as download_archive, patch(
            patch_path + ".Command.download_current"
        ) as download_current, patch(patch_path + ".notify_slack") as notify_slack:
            ...
            exp_msg = "\n".join(
                ["Imported 2 new concessions", "There are 1 unmatched concessions"]
            )
