...
class SmokeTestBase(unittest.TestCase):
    ...
    def _run_tests(self, test_name, path_fragment, params):
        ...
        for i, row in enumerate(all_rows):
            self.assertEqual('%.2f' % float(row['actual_cost']),
                             expected['cost'][i])
...
class TestSmokeTestSpendingByOnePractice(SmokeTestBase):
    def test_presentation_by_one_practice(self):
        params = {
            'code': '0703021Q0BBAAAA',
            'org': 'A81015'
        }  # Cerazette 75mcg.
        self._run_tests('presentation_by_one_practice', 'spending_by_practice',
                        params)
    ...
    def test_chemical_by_one_practice(self):
        ...
        self._run_tests('chemical_by_one_practice', 'spending_by_practice',
                        params)
    ...
    def test_multiple_chemicals_by_one_practice(self):
        ...
        self._run_tests('multiple_chemicals_by_one_practice',
                        'spending_by_practice', params)

    def test_bnf_section_by_one_practice(self):
        params = {'code': '0304', 'org': 'L84077'}
        self._run_tests('bnf_section_by_one_practice', 'spending_by_practice',
                        params)
...
class TestSmokeTestMeasures(SmokeTestBase):
    ...
    def test_measure_by_practice(self):
        q = self.retrieve_data_for_measure('ktt3_lipid_modifying_drugs',
                                           'A81001')
        bsa = {'numerator': 34, 'denominator': 1265, 'calc_value': '2.688'}
    ...
    def test_total_measures(self):
        ...
        for m in result['measures']:
            ...
            msg = "Expected last date of %s for %s; got %s" % (expected,
                                                               m['id'],
                                                               last_date)
