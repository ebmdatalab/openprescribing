    ...
    def _run_tests(self, test_name, path_fragment, params):
        ...
        for i, row in enumerate(all_rows):
            ...
            self.assertAlmostEqual(
                float(row["actual_cost"]), float(expected["cost"][i]), places=2
            )
            self.assertEqual(int(row["items"]), expected["items"][i])
            self.assertEqual(int(row["quantity"]), expected["quantity"][i])
...
class TestSmokeTestSpendingByOnePractice(SmokeTestBase):
    def test_presentation_by_one_practice(self):
        ...
        self._run_tests("presentation_by_one_practice", "spending_by_practice", params)

    def test_chemical_by_one_practice(self):
        params = {"code": "0212000AA", "org": "F84747"}  # Rosuvastatin Calcium.
        self._run_tests("chemical_by_one_practice", "spending_by_practice", params)
    ...
    def test_multiple_chemicals_by_one_practice(self):
        ...
        self._run_tests(
            "multiple_chemicals_by_one_practice", "spending_by_practice", params
        )

    def test_bnf_section_by_one_practice(self):
        params = {"code": "0304", "org": "L84077"}
        self._run_tests("bnf_section_by_one_practice", "spending_by_practice", params)
...
class TestSmokeTestMeasures(SmokeTestBase):
    ...
    def get_data_for_q3_2015(self, data):
        ...
        for d in data:
            if (
                (d["date"] == "2015-07-01")
                or (d["date"] == "2015-08-01")
                or (d["date"] == "2015-09-01")
            ):
    ...
    def test_measure_by_practice(self):
        ...
        bsa = {"numerator": 30, "denominator": 577, "calc_value": "5.199"}
        self.assertEqual(q["numerator"], bsa["numerator"])
        ...
        bsa = {"numerator": 44, "denominator": 64, "calc_value": "68.750"}
        self.assertEqual(q["numerator"], bsa["numerator"])
        self.assertEqual(q["denominator"], bsa["denominator"])
        self.assertEqual("%.3f" % q["calc_value"], bsa["calc_value"])

        q = self.retrieve_data_for_measure("ktt13_nsaids_ibuprofen", "A81001")
        bsa = {"denominator": 413, "numerator": 57, "calc_value": "13.801"}
        self.assertEqual(q["numerator"], bsa["numerator"])
        self.assertEqual(q["denominator"], bsa["denominator"])
        self.assertEqual("%.3f" % q["calc_value"], bsa["calc_value"])
    ...
    def test_total_measures(self):
        ...
        for m in result["measures"]:
            ...
            self.assertEqual(last_date, expected, msg)
