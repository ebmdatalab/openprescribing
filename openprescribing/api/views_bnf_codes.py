...
def _get_matching_products(codes, is_exact):
    ...
    for code in codes:
        if is_exact:
            ...
            chemicals = (
                Chemical.objects.filter(Q(bnf_code=code) | Q(chem_name=code))
                .filter(is_current=True)
                .extra(select={"type": 1})
                .order_by("chem_name")
            )
            products = (
                Product.objects.filter(Q(bnf_code=code) | Q(name=code))
                .filter(is_current=True)
                .extra(select={"type": 2})
                .order_by("name")
            )
            presentations = (
                Presentation.objects.current()
                .filter(Q(bnf_code=code) | Q(name=code))
                .filter(is_current=True)
                .extra(select={"type": 3})
                .order_by("name")
            )
            ...
            chemicals = (
                Chemical.objects.filter(
                    Q(bnf_code__startswith=code) | Q(chem_name__icontains=code)
                )
                .filter(is_current=True)
                .extra(select={"type": 1})
                .order_by("chem_name")
            ...
            products = (
                Product.objects.filter(
                    Q(bnf_code__startswith=code) | Q(name__icontains=code)
                )
                .filter(is_current=True)
                .extra(select={"type": 2})
                .order_by("name")
            ...
            presentations = (
                Presentation.objects.current()
                .filter(Q(bnf_code__startswith=code) | Q(name__icontains=code))
                .filter(is_current=True)
                .extra(select={"type": 3})
                .order_by("name")
            )
