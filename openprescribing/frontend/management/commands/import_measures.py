...
class Command(BaseCommand):
    ...
    def parse_options(self, options):
        ...
        if 'month' in options and options['month']:
            ...
            options['start_date'] = "%s-%02d-%02d" % (l.current_at.year - 5,
                                                      l.current_at.month,
                                                      l.current_at.day)
...
class MeasureCalculation(object):
    ...
    def full_table_name(self):
        ...
        return "[%s:%s.%s]" % (settings.BQ_PROJECT,
                               settings.BQ_MEASURES_DATASET, self.table_name())
    ...
    def add_percent_rank(self):
        ...
        return self.insert_rows_from_query(
            'percent_rank', self.table_name(), context, legacy=True)
    ...
    def _query_and_write_global_centiles(self, query_id, extra_select_sql):
        ...
        self.insert_rows_from_query(
            query_id, self.globals_table_name(), context, legacy=True)
...
class GlobalCalculation(MeasureCalculation):
    def calculate_global_cost_savings(self, practice_table_name,
                                      ccg_table_name):
    ...
    def write_global_centiles_to_database(self):
        ...
        for d in self.get_rows_as_dicts(self.globals_table_name()):
            ...
            mg, _ = MeasureGlobal.objects.get_or_create(
                measure_id=self.measure_id, month=d['month'])
...
class PracticeCalculation(MeasureCalculation):
    ...
    def calculate_practice_ratios(self):
        ...
        context = {
            'numerator_from':
            self.measure['numerator_from'],
            'numerator_where':
            numerator_where,
            'numerator_columns':
            " ".join(utils.get_columns_for_select(self.measure, 'numerator')),
            'denominator_columns':
            " ".join(
                utils.get_columns_for_select(self.measure, 'denominator')),
            'denominator_from':
            self.measure['denominator_from'],
            'denominator_where':
            denominator_where,
            'numerator_aliases':
            numerator_aliases,
            ...
            aliased_numerators,
            'practices_from':
            self.full_practices_table_name(),
            'start_date':
            self.start_date,
            'end_date':
            self.end_date
        ...
        self.insert_rows_from_query('practice_ratios',
    ...
    def calculate_global_centiles_for_practices(self):
        ...
        self._query_and_write_global_centiles('global_deciles_practices',
                                              extra_select_sql)
    ...
    def write_practice_ratios_to_database(self):
        ...
        """
        fieldnames = [
            'pct_id', 'measure_id', 'num_items', 'numerator', 'denominator',
            'month', 'percentile', 'calc_value', 'denom_items',
            'denom_quantity', 'denom_cost', 'num_cost', 'num_quantity',
            'practice_id', 'cost_savings'
        ]
...
class CCGCalculation(MeasureCalculation):
    ...
    def calculate_global_centiles_for_ccgs(self):
        ...
        self._query_and_write_global_centiles('global_deciles_ccgs',
                                              extra_select_sql)
