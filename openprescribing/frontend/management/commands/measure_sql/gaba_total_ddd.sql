...
FROM (
  ...
  FROM
    ebmdatalab.measures.pregabalin_total_mg AS rx --this is from the existing pregabalinmg query, so divide by 300 to get DDD for pregabalin https://www.whocc.no/atc_ddd_index/?code=N03AX16
  GROUP BY
    month,
    practice,
    pct)
