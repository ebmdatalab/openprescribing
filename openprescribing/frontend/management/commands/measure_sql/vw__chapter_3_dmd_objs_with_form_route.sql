-- This SQL is checked in to the git repo at measure_sql/vw__chapter_3_dmd_objs_with_dose_route.sql.
-- Do not make changes directly in BQ!  Instead, change the version in the repo and run
--
--     ./manage.py create_bq_measure_views
--
-- This SQL finds all dm+d objects in BNF chapter 3, and links each object against its
-- VMP's form_route.

WITH vmps AS (
  -- Find all VMPs in BNF chapter 3
  SELECT
    'vmp' AS obj_type,
    1 AS order_key,
    id AS vpid,
