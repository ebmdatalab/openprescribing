...
class MeasureValueManagerTests(TestCase):
    ...
    def test_by_regional_team_with_orgs(self):
        mvs = MeasureValue.objects.by_org(
            "regional_team", "regional_team", ["Y01", "Y02"]
        )
        self.assertEqual(len(mvs), 2)
    ...
    def test_by_org_with_tags(self):
        mvs = MeasureValue.objects.by_org(
            "practice", "practice", ["C83051"], tags=["core", "lowpriority"]
        )
        self.assertEqual(len(mvs), 0)

    def test_aggregate_by_measure_and_month(self):
        results = (
            MeasureValue.objects.filter(measure_id="cerazette")
            .by_org("practice")
...
class CalculateCostSavingsTests(TestCase):
    ...
    def test_calculate_cost_savings(self):
        ...
        month = "2017-10-01"
        target_costs = (
            MeasureGlobal.objects.filter(measure_id=measure_id, month=month)
            .get()
            .percentiles["ccg"]
        )
        cost_savings = (
            MeasureValue.objects.filter(measure_id=measure_id, month=month)
