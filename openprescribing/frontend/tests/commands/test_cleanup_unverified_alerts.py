...
class CommandsTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.approved_count = (
            self.approved_orgbookmark_count + self.approved_searchbookmark_count
        )
        self.unapproved_count = (
            self.unapproved_orgbookmark_count + self.unapproved_searchbookmark_count
        )

    def test_old_unapproved_bookmarks_deleted(self):
        # The fixtures contain one unapproved bookmark
        self.assertEqual(
            OrgBookmark.objects.count(),
            self.approved_orgbookmark_count + self.unapproved_orgbookmark_count,
        )
        self.assertEqual(
            SearchBookmark.objects.count(),
            self.approved_searchbookmark_count + self.unapproved_searchbookmark_count,
        )
        call_command("cleanup_unverified_alerts")
        self.assertEqual(OrgBookmark.objects.count(), self.approved_orgbookmark_count)
        self.assertEqual(
            SearchBookmark.objects.count(), self.approved_searchbookmark_count
        )
    ...
    def _makeUser(
        self, has_bookmarks=True, date_joined=None, is_superuser=None, verified=None
    ):
        if has_bookmarks:
