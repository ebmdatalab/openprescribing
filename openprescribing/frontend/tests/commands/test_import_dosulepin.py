...
class CommandsTestCase(TestCase):
    ...
    def test_import_dosulepin(self):
        ...
        self.assertEqual(mv.calc_value, 0.0)
        self.assertEqual(mv.percentile, 0)

        mg = MeasureGlobal.objects.get(measure=m, month=month)
        self.assertEqual(mg.numerator, 68)
        self.assertEqual(mg.denominator, 451)
        self.assertEqual("%.4f" % mg.calc_value, '0.1508')
        self.assertEqual("%.4f" % mg.practice_10th, '0.0164')
        self.assertEqual("%.4f" % mg.practice_90th, '0.7719')
