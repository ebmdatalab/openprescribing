...
class MailLogTestCase(TestCase):
    def test_metadata_nests_correctly(self):
        MailLog.objects.create(
            recipient="me", event_type="accepted", metadata={"thing": ["foo"]}
        )
    ...
    def test_bounced_subject_no_stored_email(self):
        MailLog.objects.create(
            recipient="me", event_type="bounced", metadata={}, message_id="123"
        )
        self.assertEqual(MailLog.objects.first().subject_from_metadata(), "n/a")

    def test_bounced_subject_no_message_id(self):
        MailLog.objects.create(recipient="me", event_type="bounced", metadata={})
        self.assertEqual(MailLog.objects.first().subject_from_metadata(), "n/a")
...
class TestPresentationDMDLinks(TestCase):
    ...
    def test_dmd_info(self):
        ...
        info = p.dmd_info()
        self.assertEqual(
            info["prescribability_statuses"], "Valid as a prescribable product"
        )
