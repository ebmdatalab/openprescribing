...
class GeneralFrontendTest(SeleniumTestCase):
    ...
    def test_menu_dropdown_on_doc_page(self):
        with patch('requests.get') as mock_response:
            ...
            text = PropertyMock(return_value=('<head><style></style></head>'
                                              '<body>some text</body>'))
            ...
            self.assertTrue(
                self.find_by_xpath("//a[contains(text(), 'About')]")
                .is_displayed(), "dropdown functionality broken at %s" % url)

    def test_menu_dropdown(self):
        for url in [
                '/ccg/03Q/', '/practice/P87629/', '/measure/cerazette/',
                '/chemical/0202010D0/', '/bnf/020201/', '/analyse/',
                '/measure/', '/about'
        ]:
    ...
    def test_drug_name_typeahead(self):
        ...
        el = self.find_by_xpath('//div[@id="denomIds-wrapper"]'
    ...
    def test_practice_name_typeahead(self):
        ...
        el = self.find_by_xpath('//div[@id="orgIds-container"]'
                                '//input[@class="select2-search__field"]')
        el.send_keys("ains")
        # This should succeed; if not, the JSON dropdown-filling has not:
    ...
    def test_ccg_measures_sorting(self):
        ...
        self.assertEqual(
            self.find_by_xpath("//div[@id='charts']/div[1]").get_attribute(
                "id"), 'measure_cerazette')
    ...
    def test_ccg_measures_tags(self):
        ...
        time.sleep(1)
        self.assertTrue(
            self.find_by_xpath("//p[contains(text(), 'prescribed on any')]"))
    ...
    def test_ccg_measures_explore_link(self):
        ...
        self.assertIn(
            '/measure/keppra',
            measure.find_element_by_partial_link_text("Compare all CCGs")
            .get_attribute('href'),
        )
        self.assertIn(
            '/ccg/02Q/keppra',
            measure.find_element_by_partial_link_text("Split the measure")
            .get_attribute('href'),
        )
