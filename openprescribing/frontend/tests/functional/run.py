...
class FrontendTest(unittest.TestCase):
    ...
    def test_form_and_chart_render_from_url(self):
        ...
        self.assertEqual('0212000B0AAACAC', denom_val[0].get_attribute("value"))
        self.assertEqual('0212000B0AAADAD', denom_val[1].get_attribute("value"))
        ...
        self.assertEqual('Spend on 0212000B0AAACAC + 0212000B0AAADAD', xaxis_title.text)
        ...
        scale_button = self.browser.find_element_by_css_selector('#useLogScale .btn-primary')
        ...
        chart_points = self.browser.find_elements_by_css_selector('.highcharts-markers path')
    ...
    def test_form_and_chart_render_from_url_with_list_size(self):
        ...
        # Chart title, axis titles, axis labels
        title = self.browser.find_element_by_css_selector('.highcharts-title')
        expected_title = 'Total spending by practice on 0212000AA since August 2010'
    ...
    def test_form_autocomplete(self):
        ...
        num_type = self.browser.find_element_by_id('numerator')
        self.assertEqual('bnf-section', num_type.get_attribute("value"))
        num_val = self.browser.find_element_by_css_selector('#numerator-row .tt-input')
        self.assertEqual('030401', num_val.get_attribute("value"))

        # Test BNF section autocomplete
        num_val.clear()
        num_val.send_keys("4.2")
        suggestions = self.browser.find_elements_by_css_selector('#numerator-row .tt-suggestion')
        ...
        num_chosen.click()
        num_options = self.browser.find_elements_by_css_selector('#numerator_chosen li')
        num_options[0].click()
        num_val = self.browser.find_element_by_css_selector('#numerator-row .tt-input')
        num_val.click()
        ...
        self.assertEqual(suggestions[0].text, 'Amoxicillin_Cap 250mg (0501013B0AAAAAA)')

        # Test chemical autocomplete
        denom_val_css = '#denominator-row .tt-input'
        denom_val = self.browser.find_element_by_css_selector(denom_val_css)
        denom_val.send_keys("0501013B0")
        suggestions = self.browser.find_elements_by_css_selector('#denominator-row .tt-suggestion')
        ...
        # empty_message = self.browser.find_element_by_css_selector('.empty-message')
        # self.assertEqual(empty_message.text, 'No matches found')
    ...
    def test_form_submit(self):
        ...
        denom_val = self.browser.find_element_by_css_selector('#denominator-row .tt-input')
        denom_val.send_keys("0501013B0")
        suggestions = self.browser.find_elements_by_css_selector('#denominator-row .tt-suggestion')
        suggestions[0].click()
        ...
        title = self.browser.find_element_by_css_selector('.highcharts-title')
        self.assertEqual('Total spending by CCG on Ampicillin (0501013E0)vs. spending on Amoxicillin (0501013B0) since April 2013', title.text)
    ...
    def test_form_add_buttons(self):
        ...
        self.browser.find_element_by_css_selector('#numerator-row .add').click()
        num_inputs = self.browser.find_elements_by_css_selector('#numerator-row .tt-input')
        ...
        self.browser.find_element_by_css_selector('#denominator-row .add').click()
        denom_inputs = self.browser.find_elements_by_css_selector('#denominator-row .tt-input')
        ...
        denom_minus = self.browser.find_elements_by_css_selector('#denominator-row .minus')
        denom_minus[1].click()
        denom_inputs = self.browser.find_elements_by_css_selector('#denominator-row .tt-input')
    ...
    def test_chart_linear_log_scale_button_works(self):
        ...
        # TODO: Test with a chart with lots of zero values, e.g.
        # http://localhost:8000/analyse/#org=practice&numerator=chemical&numeratorIds=0212000AA&denominator=list-size&denominatorIds=&period=all-time&scale=linear&
    ...
    def test_chart_slider(self):
        ...
        slider = self.browser.find_elements_by_css_selector('#findItemWrapper .tt-suggestion')
        self.assertEqual(len(suggestions), 3)
