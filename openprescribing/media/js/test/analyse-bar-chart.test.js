import { describe, expect, it } from "vitest";
import barChart from "../src/analyse-bar-chart";

describe("BarChart", () => {
  describe("#_indexDataByMonthAndRatio", () => {
    it("should construct the bar chart series", () => {
      const combinedData = [
        {
          ratio_actual_cost: 12,
          y_actual_cost: 105,
          x_actual_cost: 10,
          id: "03Q",
          name: "NHS Corby",
          date: "2014-03-01",
        },
        {
          ratio_actual_cost: 10,
          y_actual_cost: 204,
          x_actual_cost: 11,
          id: "03V",
          name: "NHS Vale of York",
          date: "2014-03-01",
        },
        {
          ratio_actual_cost: 15,
          y_actual_cost: 128,
          x_actual_cost: 12,
          id: "03Q",
          name: "NHS Corby",
          date: "2014-04-01",
        },
        {
          ratio_actual_cost: 9,
          y_actual_cost: 179,
          x_actual_cost: 13,
          id: "03V",
          name: "NHS Vale of York",
          date: "2014-04-01",
        },
      ];
      const indexedData = barChart._indexDataByMonthAndRatio(combinedData, [
        "03Q",
      ]);

      const costs = indexedData.barData["2014-03-01"].ratio_actual_cost;
      expect(costs.length).to.equal(2);
      expect(costs[1].id).to.equal("03Q");
      expect(costs[1].y).to.equal(12);
      expect(costs[1].y_actual_cost).to.equal(105);
      expect(costs[1].x_actual_cost).to.equal(10);
      expect(costs[1].date).to.equal("2014-03-01");
    });

    it("should colour bars with active IDs", () => {
      const combinedData = [
        {
          ratio_actual_cost: 12,
          y_actual_cost: 105,
          x_actual_cost: 10,
          id: "03Q",
          name: "NHS Corby",
          date: "2014-03-01",
        },
        {
          ratio_actual_cost: 10,
          y_actual_cost: 204,
          x_actual_cost: 11,
          id: "03V",
          name: "NHS Vale of York",
          date: "2014-03-01",
        },
        {
          ratio_actual_cost: 15,
          y_actual_cost: 128,
          x_actual_cost: 12,
          id: "03Q",
          name: "NHS Corby",
          date: "2014-04-01",
        },
        {
          ratio_actual_cost: 9,
          y_actual_cost: 179,
          x_actual_cost: 13,
          id: "03V",
          name: "NHS Vale of York",
          date: "2014-04-01",
        },
      ];
      const indexedData = barChart._indexDataByMonthAndRatio(combinedData, [
        "03Q",
      ]);
      const costs = indexedData.barData["2014-03-01"].ratio_actual_cost;
      expect(costs[1].color).to.equal("rgba(255, 64, 129, .8)");
      expect(costs[0].color).to.equal("rgba(119, 152, 191, .5)");
      expect(indexedData.maxRatioItems).to.equal(0);
      expect(indexedData.maxRatioActualCost).to.equal(15);
    });

    it("should sort the series by ratio_items ascending", () => {
      const combinedData = [
        {
          ratio_items: 12,
          y_actual_cost: 105,
          x_actual_cost: 10,
          id: "O3Q",
          name: "NHS Corby",
          date: "2014-03-01",
        },
        {
          ratio_items: 10,
          y_actual_cost: 204,
          x_actual_cost: 11,
          id: "O3V",
          name: "NHS Vale of York",
          date: "2014-03-01",
        },
        {
          ratio_items: 15,
          y_actual_cost: 128,
          x_actual_cost: 12,
          id: "O3Q",
          name: "NHS Corby",
          date: "2014-04-01",
        },
        {
          ratio_items: 9,
          y_actual_cost: 179,
          x_actual_cost: 13,
          id: "O3V",
          name: "NHS Vale of York",
          date: "2014-04-01",
        },
      ];
      const indexedData = barChart._indexDataByMonthAndRatio(combinedData, []);
      const items = indexedData.barData["2014-03-01"].ratio_items;
      expect(items.length).to.equal(2);
      expect(items[0].id).to.equal("O3V");
      expect(items[0].y).to.equal(10);
      expect(indexedData.maxRatioItems).to.equal(15);
      expect(indexedData.maxRatioActualCost).to.equal(0);
    });
  });
});
