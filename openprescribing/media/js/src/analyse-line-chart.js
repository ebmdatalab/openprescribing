import _ from "underscore";
import formatters from "./chart_formatters";

const lineChart = {
  setUp(lineOptions, globalOptions) {
    lineOptions.title.text = globalOptions.friendly.chartTitle;
    lineOptions.yAxis.title = {
      text: globalOptions.friendly.yAxisTitle,
    };
    if (lineOptions.yAxis.title.text.includes("<br/>")) {
      lineOptions.yAxis.title.margin = 35;
    }
    lineOptions.tooltip = {
      formatter() {
        const series_name = this.series.name;
        const month = this.x;
        const original_x = this.point.original_x;
        const original_y = this.point.original_y;
        const y = this.y;
        // Set force_items=true here, because this chart
        // only ever shows items, not spending.
        // Dynamic updates to the chart are too slow.
        return formatters.constructTooltip(
          globalOptions,
          series_name,
          month,
          original_y,
          original_x,
          y,
          true
        );
      },
    };
    lineOptions.series = this.getDataForLineChart(
      globalOptions.data.combinedData,
      globalOptions.chartValues,
      _.pluck(globalOptions.orgIds, "id")
    );
    return new Highcharts.Chart(lineOptions);
  },

  getDataForLineChart(combinedData, { y, x_val, ratio }, activeOrgs) {
    // We want each org to be a series, with an id, name and data[] property.
    // We want each point in the data array to have an x (date) and y (number) property.
    // The y property should be the ratio that we care about.
    // console.log('getDataForLineChart', combinedData, chartValues, activeOrgs);
    const dataForOrganisation = {};

    const allIds = [];
    const series = [];
    let hasActiveOrg;
    _.each(combinedData, (d) => {
      if (!(d.id in dataForOrganisation)) {
        dataForOrganisation[d.id] = {
          id: d.id,
          name: d.name,
          data: [],
        };
      }
      const dataPoint = {};
      const dates = d.date.split("-");
      if (d.id.length === 3 && d.date < "2013-08-01") {
        // Skip CCGs for months before they existed. As we keep just 5
        // years of data, this special casing can be removed after
        // August 2018.
      } else {
        // Use date of previous month to calculate x value
        const date = Date.UTC(dates[0], dates[1] - 1, dates[2]);
        dataPoint.original_y = d[y];
        dataPoint.original_x = d[x_val];
        dataPoint.x = date;
        dataPoint.y = d[ratio];
        dataForOrganisation[d.id].data.push(dataPoint);
        allIds.push(d.id);
      }
    });
    hasActiveOrg = _.intersection(allIds, activeOrgs).length !== 0;
    for (const k in dataForOrganisation) {
      const s = dataForOrganisation[k];
      if (hasActiveOrg) {
        if (_.contains(activeOrgs, s.id)) {
          s.color = "rgba(255, 64, 129, 0.6)";
          s.zIndex = 1000;
        } else {
          s.color = "rgba(204, 204, 204, 0.6)";
          s.zIndex = 1;
        }
      }
      series.push(s);
    }
    return series;
  },
};

export default lineChart;
