...
# Newrelic Apps
NEWRELIC_APPIDS = {"production": "45170403", "staging": "45937313", "test": "45170011"}


def notify_newrelic(revision, url, fab_env="production"):
    payload = {"deployment": {"revision": revision, "changelog": url}}
    app_id = NEWRELIC_APPIDS[fab_env]
    headers = {"X-Api-Key": os.environ["NEWRELIC_API_KEY"]}
    response = requests.post(
        ("https://api.newrelic.com/v2/applications/" "%s/deployments.json" % app_id),
        headers=headers,
        json=payload,
    )
